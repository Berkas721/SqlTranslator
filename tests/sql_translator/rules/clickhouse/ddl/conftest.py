"""Хелперы для модульных тестов DDL-правил clickhouse.
"""
...
from sql_translator.ast.registry import TranslateContext, default_translator
...
@pytest.fixture
def make():
    """Фабрика PG-узла нужного класса с проставленными node_kind и dialect."""
...
@pytest.fixture
def apply(ctx):
    """Применить все подходящие правила к узлу (target=CLICKHOUSE)."""
    def _apply(node):
...
@pytest.fixture
def kinds():
    """Список Kind аннотаций узла."""
    return lambda n: [a.kind for a in n.annotations]


@pytest.fixture
def rule_ids():
    """Список rule_id аннотаций узла."""
